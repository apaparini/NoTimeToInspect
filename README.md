**analysis_dofile** contains the Stata scripts for the replication of the analysis, including robustness checks and charts.

**compile_cbr** contains the Stata scripts to rearrange the CBR-LRI from individual sheets into a combined, long format pannel. The original dataset can be downloaded from:

        Armour, J., Deakin, S., &amp; Siems, M. (2016). CBR Leximetric Datasets. University of Cambridge. https://doi.org/10.17863/CAM.506

**overall_trends** is Figure II of the paper, showcasing the evolution of the standardized means of the CBR-LRI (indicators 1-31) and the number of inspectors per 100K employed, between 2009-2022.

**scatterplot_burdencapacity_coverage** is Figure III of the paper, showcasing the country average values and the regression line, for the period 2009-2022.

Additional sources used to build the dataset were:

  - _V-Dem-16_ (2026) as available in https://v-dem.net/data/the-v-dem-dataset/
  - _ILOSTAT_ (2026) as available in https://rshiny.ilo.org/dataexplorer98/
  - _World Bank Country Classifications_ as available in https://datahelpdesk.worldbank.org/knowledgebase/articles/378834-how-does-the-world-bank-classify-countries
